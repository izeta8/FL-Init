...
This guide documents the steps required to compile, package, and publish a new version of **FL-Init** to GitHub for Windows, allowing existing users to receive automatic updates via `electron-updater`.
...
Open the [package.json](../package.json) file and update the `"version"` field value following [SemVer](https://semver.org/) rules (e.g., from `1.0.5` to `2.0.0` for a major refactor or breaking change).
```json
{
  "name": "flinit",
  "version": "2.0.0",
...
Ensure you have configured a valid GitHub access token in your local [.env](../.env) file:
...
```
This script downloads a clean Windows portable Python runtime and installs all requirements in the distribution folder `dist/python`.

### Step 4: Compile and Upload the Executables
Run the automated publication script from the Windows PowerShell/CMD:
```bash
npm run publish
```
This command performs the following tasks automatically:
1.  Loads the `GH_TOKEN` environment variable from your `.env` file.
2.  Compiles the main process TypeScript files (`src/main`) and packages the React frontend using Webpack in production mode.
...
2.  You will find a new release marked as **Draft** with your version number (e.g., `v2.0.0`) and the installer file (`.exe`) attached as an asset.
...
The application is integrated with an automatic updater system configured in [updater.ts](../src/main/updater.ts):
...
3.  It downloads the updated `.exe` installer in the background and restarts the application to apply it.

---

## ⚠️ Troubleshooting

### Error: `GitHub Personal Access Token is not specified` or `401 Unauthorized`
*   **Cause:** The `GH_TOKEN` is missing, incorrect, or has expired.
*   **Solution:** Check your `.env` file and make sure there are no extra spaces. Verify that the token is still active on GitHub.
